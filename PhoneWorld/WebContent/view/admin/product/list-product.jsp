			...
			<div id="content">
				...
				<nav
					...
					<form
						...
						method="post" action="/PhoneWorld/admin/product/list">
								...
								<tbody>
									<c:forEach items="${proList}" var="proList">
										<tr>
											...
											<td>${proList.CPU }</td>
											<td>${proList.pin }</td>
											<td>${proList.brand.getBrand() }</td>
											<c:url value="/product/download?filename=${proList.image }"
												var="imgUrl"></c:url>
											<td><img height="150" src="${imgUrl}" /></td>
											<td><a
												href="<c:url value='/admin/product/edit?id=${proList.id }'/>"
												class="hvr-grow label label-warning">Edit</a> <a
												href="<c:url value='/admin/product/delete?id=${proList.id }'/>"
