			...
			<div id="content">
				...
				<div class="chit-chat-layer1 bg">
					<div class="work-progres">
						<div class="chit-chat-heading">Table Orders</div>
						...
						<div class="table-responsive">
							<table class="table table-hover">
								...
								<tbody>
									<c:forEach items="${tableOrdersList }" var="cartItemList"
										varStatus="status">
										<tr>
											<td>${status.count }</td>
											<td>${cartItemList.cartItem.id }</td>
											<c:url
												value="/product/download?filename=${cartItemList.cartItem.product.image }"
												var="imgUrl"></c:url>
											<td><img height="150" src="${imgUrl}" /></td>
											<td>${cartItemList.cartItem.product.id }</td>
											<td>${cartItemList.cartItem.product.productName }</td>
											<td>${cartItemList.cartItem.buyerName }</td>
											<td>${cartItemList.cartItem.buyerPhone }</td>
											<td>${cartItemList.cartItem.buyerAddress }</td>
											<td>${cartItemList.cartItem.quantity }</td>
											<td>${cartItemList.cartItem.product.price }</td>
											<td>${cartItemList.cartItem.product.price * cartItemList.cartItem.quantity }</td>
											<td>${cartItemList.date }</td>
											<td><a
												href="<c:url value='/admin/order-table/delete?tableOrderId=${cartItemList.cartItem.id}'/>">Delete</a>
											</td>
										</tr>
									</c:forEach>
								</tbody>
